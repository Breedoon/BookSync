...
def infer_transcript_timing(probs: np.array, alph: np.array, beam_width=25, transcript=None, **kwargs):
    trpt_inds = pd.Series(np.arange(len(alph)), index=alph)[list(transcript)]
    p_fit = np.zeros((len(transcript), len(probs)))
    for trpt_t, trpt_c in enumerate(trpt_inds):
        for rec_t, rec_c_dist in enumerate(probs):
            p_fit[trpt_t, rec_t] = rec_c_dist[trpt_c]
